#!/bin/bash
# Быстрый старт CRM системы batmGAMES
...
pip install gspread google-auth || echo "⚠️ Ошибка установки. Попробуй: uv sync"

# 2. Применение миграций
echo ""
echo "[2/4] Применение миграций БД..."
python -m app.db.migrate || echo "⚠️ Ошибка миграций"

# 3. Синхронизация клиентов
echo ""
echo "[3/4] Первая синхронизация клиентов → Google Sheets..."
python -m app.services.sheets_sync || echo "⚠️ Ошибка синхронизации клиентов"

# 4. Синхронизация заказов
echo ""
echo "[4/4] Первая синхронизация заказов → Google Sheets..."
python -m app.services.sync_orders || echo "⚠️ Ошибка синхронизации заказов"
